import { Controller, Post, Get, UseInterceptors, UploadedFiles, Body, BadRequestException, UseGuards, Req } from '@nestjs/common';
...
export class MarketingExecutiveController {
  constructor(private readonly marketingExecutiveService: MarketingExecutiveService) { }
    ...
    FileFieldsInterceptor([
      ...
      fileFilter: (req, file, callback) => {
        if (!file.mimetype.match(/\/(jpg|jpeg|png)$/)) {
          return callback(new BadRequestException('Only image files (png/jpeg/jpg) are allowed!'), false);
        }
        callback(null, true);
      },
    }),
  ...
  async createLead(
    ...
    try {
      ...
      return await this.marketingExecutiveService.createLead(createDto, files, userId);
    } catch (error) {
      if (files) {
        const allFiles = [
